...
- **`contentOffset` is not the payload start for value atoms (2026-07):** `BoxNode.ContentOffset` only accounts for the ISO FullBox 4-byte prefix. The `data`, `mean`, and freeform `name` atoms carry an additional value prefix (8 bytes for `data`, 4 for `mean`/`name`) that the parser reads manually, outside `IsFullBox`, so `ContentOffset` points BEFORE the real value for those atoms. The `clip_get_boxtree` surface deliberately does not expose it; it publishes `offset`, `size`, and `headerSize` (honest box geometry) only. If you ever add a "payload start" field, special-case those three atom types.
