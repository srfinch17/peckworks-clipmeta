...
### clipmetamcp
An MCP (Model Context Protocol) server exposing the clipmeta tools to MCP hosts such as Claude Desktop, so you can read and tag clips conversationally. Ships as a self-contained `.mcpb` bundle (built by `tools/pack-mcpb.ps1`) — drag onto Claude Desktop → Settings → Extensions and pick your clips folder; no .NET install needed on the target machine. All file access is sandboxed to the chosen folder.
