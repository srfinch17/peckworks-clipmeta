...
## Architecture (as of 2026-07, v1.1.0)
...
| `clipmetaview` | `ClipMetaView` | Thin CLI: renders the box/atom tree, plus `--json` (structured box tree) and `--definitions` (box-type dictionary). References Core. |
...
| `clipmetamcp` | `ClipMetaMcp` | Thin MCP server shell: stdio JSON-RPC 2.0, exposes 18 clipmeta tools to MCP hosts (Claude Desktop). References Core. Packs to a `.mcpb` bundle via `tools/pack-mcpb.ps1`. Product version comes from the repo-root `VERSION` file (see Versioning). The watched-clip resolver is at pass-7; v1.1.0 added the `clip_get_boxtree` read tool. |
| `clipmetaview.Tests` | | MSTest, 115 tests (112 passing, 3 skipped without local clips). |
| `clipmetascribe.Tests` | | MSTest, 562 tests (incl. real-clip integration and byte-level media-integrity tests). |
| `clipmetamcp.Tests` | | MSTest, 141 tests (protocol shape, tool behavior, sandbox escapes, stdout purity). |
...
> `TreeRenderer` now lives in `clipmeta.core/Rendering/`, shared by the CLI and the `clip_get_boxtree` MCP tool.

> **v1.0.1 hardening (2026-07):** every write funnels through `Mp4Writer.WriteMetadata`, which runs inside a per-file `CrossProcessLock` (named mutex keyed on the canonicalized path; `WriteGate` and `TagQueue` delegate to it, lock order is queue-then-file) and passes refusal gates before any byte is written: `DetectFragmented`, `DetectMissingMoov`, whole-file accounting, and `DetectNonCanonicalMetadata` (refuses clipmeta-namespaced atoms outside `moov.udta.meta.ilst`; foreign metadata elsewhere, camera GPS, iTunes atoms, Xtra, is preserved by design). Post-write verification compares read-back VALUES at the canonical path, not just atom existence. Directory scanners skip unreadable files and NAME them (the MCP scan tools carry an additive `skipped` field). The parser stays deliberately LENIENT on truncated files (reads yield an empty tree, pinned by tests); refusal is a write-path concern. Consult PITFALLS before changing either side.
...
A self-contained GitHub Pages info/landing page lives at `docs/index.html` (build record + decisions in `docs/BUILD-LOG.md`) and is published at https://srfinch17.github.io/peckworks-clipmeta/ (GitHub Pages serving `main` /docs, with `docs/.nojekyll` so it's served verbatim instead of through Jekyll/README). Attribution is "Peckworks Lab" (no personal-name placeholder). The two former "screenshot coming soon" slots are now filled with rendered clip-metadata panels (voice-tagging in PART 04, search in PART 06), and PART 03's schematic box-tree SVG was replaced by a real before/after box tree plus a `--list` companion; a vocab figure sits beside the vocab card. All panels are CSS-namespaced under `.cm-*` with scoped variables so they cannot collide with the page or each other (their palette and JetBrains Mono are their own; only the card frame is docked to the page tokens). An actual gameplay/montage still remains optional. Treat it as a curated artifact, don't clobber or regenerate it casually.

### License (PolyForm Noncommercial 1.0.0, since 2026-09-09)
`LICENSE.md` at the repo root: free for any noncommercial use, commercial use needs a separate license from Peckworks Lab (copyright holder). This makes the project **source-available, not open source** (the OSI definition forbids use restrictions), so never describe it as "open source" anywhere public: README, landing page, release notes, manifest, commit messages. The license's Notices clause requires the terms to travel with every distributed copy, so `pack-mcpb.ps1` and `build-release-artifacts.ps1` copy `LICENSE.md` into the `.mcpb` bundle and the CLI zip; a new distributable artifact must do the same. The manifest carries the SPDX id `PolyForm-Noncommercial-1.0.0`. Changing the license is a cross-artifact edit (README, landing page, manifest, both release scripts), not a one-file swap.
...
- **MCP revision 2026-07-28 (stateless, dual-era server), SPEC APPROVED, NOT BUILT:** `docs/superpowers/specs/2026-09-09-mcp-2026-07-28-dual-era-design.md` on branch `feat/mcp-2026-07-28-dual-era`. Brainstorm is done; resume with `superpowers:writing-plans` from the spec. Decisions locked: hand-rolled protocol layer stays (no SDK), era decided per request from `_meta` (never cached on the session), legacy `initialize` path byte-for-byte unchanged, `server/discover` advertises modern versions only. Branch-only until the owner says merge.
...
- **Pre-publish grep, three tokens.** Before any push, release, or Pages update, grep the tracked tree for: the em-dash character (above), the owner's Windows username or any `C:\Users\<name>\` path (public artifacts use a name-free display path such as `C:\Users\Peckworks\...`), and the phrase "open source" (see License). Each of these has shipped once; the grep is the guard.
- **Mechanical edits get a diff, not trust.** GNU sed on this repo's PowerShell scripts has mangled a Windows path (`\L` in a replacement is sed's lowercase escape, so `'..\LICENSE.md'` became `'..icense.md'`). Use a literal-replacement editor for anything containing backslashes, and read the diff before committing regardless of what tool made the change.
...
- Run `dotnet test` in the FOREGROUND with an explicit long timeout (~10 min); do not background it and wait for a completion notification. Also: idle MSBuild `dotnet.exe` build-server nodes linger after builds and look like a running suite; an actually-running suite has `testhost` processes, check for those before concluding tests are still going.
...
- **Accepted risk: CI never exercises real MP4s.** The media-integrity suite (`clipmetascribe.Tests/Mp4WriteIntegrityTests.cs`, the `RealClip_*` tests) is the only place actual encoder output goes through the full write path, and it graceful-skips on CI because `testclips/pristine/` is git-ignored (personal gameplay footage). A small non-personal, real-encoder replacement corpus was investigated for CI (2026-07) and rejected for now: this machine has no ffmpeg (or any other encoder) on PATH, and a hand-crafted box tree would add nothing over the existing synthetic fixtures, the whole point of this suite is real-world encoder output, not a shape we built ourselves. So CI green proves the synthetic/unit/protocol/sandbox suites only, not the real-clip media path. Mitigation: real-clip integrity runs pre-release on the owner's machine, where the pristine corpus lives, see the release checklist in Versioning below. Revisit if a cheap, genuinely non-personal encoder source shows up.
...
**Cutting a release:** rewrite `.github/release-notes.md` for the new version (it becomes the published release body, see the checklist below) → `bump-version.ps1` → commit → `git tag vX.Y.Z` (the tag must match `VERSION`) → `git push origin vX.Y.Z`. The **`Release` workflow** (`.github/workflows/release.yml`) then builds the assets on a Windows runner and publishes the GitHub Release automatically, it fails the run if the tag and `VERSION` disagree. Use the Actions tab "Run workflow" (workflow_dispatch) to build + upload the assets to the run for inspection **without** publishing, to test the build before tagging. (Releases are Windows-only, self-contained, unsigned.) Note: v1.0.0 predates this workflow and was hand-published; v1.0.1 is the first release actually cut by CI.

**Pre-release checklist:**
1. `dotnet build` and `dotnet test`, all suites green (see Build & test).
2. **Run the pristine suite locally before tagging.** CI cannot exercise real MP4s (see the accepted-risk note in Build & test above), so `clipmetascribe.Tests` must be run on the owner's machine with `testclips/pristine/` populated at least once per release to actually cover the real-clip write path.
3. `check-version.ps1` shows no drift.
4. **Rewrite `.github/release-notes.md` for the new version.** This file IS the published GitHub Release body (the workflow passes it to `gh release create --notes-file`), and `bump-version.ps1` does NOT touch it, so it silently carries the PREVIOUS version's notes until you edit it. Swap the "What's new in vX.Y.Z" section (the download/install sections below it are evergreen). Missing this ships a release whose notes describe the wrong version.
5. `bump-version.ps1` → commit → tag → push, per the flow above.
