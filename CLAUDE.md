...
Solution: `peckworks-clipmeta.slnx`, **.NET 10**, four projects:
...
| `clipmetascribe.Tests` | — | MSTest, 206 tests (incl. real-clip integration). |
...
- 286 tests total. `clipmetascribe.Tests` takes ~2.5 min (real-clip integration) — not a hang; use a long timeout.
...
2. `dotnet test` — all 286 pass, including real-clip integration.
