...
## Architecture (as of 2026-07, v1.0.1)
...
| `clipmetamcp` | `ClipMetaMcp` | Thin MCP server shell: stdio JSON-RPC 2.0, exposes 18 clipmeta tools to MCP hosts (Claude Desktop). References Core. Packs to a `.mcpb` bundle via `tools/pack-mcpb.ps1`. Product version comes from the repo-root `VERSION` file (see Versioning); latest feature work is pass-7. |
| `clipmetaview.Tests` | | MSTest, 109 tests (106 passing, 3 skipped without local clips). |
| `clipmetascribe.Tests` | | MSTest, 544 tests (incl. real-clip integration and byte-level media-integrity tests). |
| `clipmetamcp.Tests` | | MSTest, 137 tests (protocol shape, tool behavior, sandbox escapes, stdout purity). |
